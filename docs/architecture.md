...
`OPENAI_API_KEY` en de optionele `TEAMS_WEBHOOK_URL` bestaan uitsluitend als GitHub Actions-secrets. De OpenAI-sleutel gaat direct naar de officiële Codex-action; de webhook alleen naar het meldingsscript. Iedere begrensde Codex-stap gebruikt zowel de read-only sandbox als de officiële `read-only` safety strategy: artikeltekst is onbetrouwbare data, de agent kan niet schrijven en heeft geen directe netwerktoegang. Deze modus kan voor opeenvolgende batches in dezelfde tijdelijke runner worden herhaald; `drop-sudo` kan volgens de action niet een tweede keer in dezelfde job worden gestart. Een mislukte run pusht niets, zodat de vorige productieversie online blijft. Een ontbrekende of ongeldige webhook schakelt alleen de meldingen uit en blokkeert de nieuwsactualisatie niet.
